...
run_player() {
    ...
    for i in $(seq 0 $[players-1]); do
      >&2 echo Running taskset -c `expr $i \* $nthreads`-`expr $i \* $nthreads + $nthreads - 1` $prefix $SPDZROOT/$bin $i $params
      log=$SPDZROOT/logs/$log_prefix$i
      taskset -c `expr $i \* $nthreads`-`expr $i \* $nthreads + $nthreads - 1` $prefix $SPDZROOT/$bin $i $params 2>&1 |
	  {
	      if test "$BENCH"; then
